...
namespace Engine3D
{
    public class AxisIndicator
    {
        ...
        public void Create(float spokeLen, float spokeRad, float centerRad, float arrowLen, float arrowRad)
        {
            ...
            // x arrow
            cn = new Cylinder3d();
            cn.Create(arrowRad, 0.0f, arrowLen, 128, 2);
            cn.SetColor(Color.Red);
            cn.Rotate(0, ang90, 0);
            cn.Translate(spokeLen + arrowLen / 2 - 0.1f, 0,  - arrowLen / 2);
            axisInd.Add(cn);
            cn = new Cylinder3d();
            cn.Create(spokeRad, spokeRad, spokeLen, 128, 2);
            cn.Rotate(0, ang90, 0);
            ...
            //sp.SetColor(UVDLPApp.Instance().Engine3D.m_supportColor);
            //axisInd.Add(sp);
            //float r = centerRad;
        ...
        public void RenderGL()
        {
            ...
            {
                obj.RenderGL(true, false, false, UVDLPApp.Instance().Engine3D.m_objectColor);
            }
        ...
        public void Move(Point3d xyz)
        {
            foreach (Object3d obj in axisInd)
            {
                obj.Translate(xyz.x, xyz.y, xyz.z);
            }
        }
        public void RenderGL(Point3d m_center)
        {
            foreach (Object3d obj in axisInd)
            {
                obj.Translate(m_center.x, m_center.y, m_center.z);
            }
        ...
        internal void RenderGL(float v1, float v2, float v3)
        {
            this.RenderGL(new Point3d(v1, v2, v3));
        }
    }
}
