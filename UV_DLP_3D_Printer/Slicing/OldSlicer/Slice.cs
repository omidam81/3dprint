...
namespace UV_DLP_3D_Printer
{
    ...
    public class Slice 
    {
        ...
        public void RenderSlice(SliceBuildConfig sp, ref Bitmap bmp, bool outlineonly = true)
        {
            try
            {
                using (Graphics graph = Graphics.FromImage(bmp))
                using (Pen pen = new Pen(UVDLPApp.Instance().m_appconfig.m_foregroundcolor, 1))
                {
                    Point pnt1 = new Point(); // create some points for drawing
                    Point pnt2 = new Point();
                    ...
                    for (int y = mm.ymin; y < mm.ymax; y++) // this needs to be in scaled value 
                    {
                        ...
                        else if (UVDLPApp.Instance().m_slicer.m_slicemethod == Slicer.eSliceMethod.eNormalCount)
                        {
                            ...
                            if (points.Count != 0)
                            {
                                ...
                                {
                                    DebugLogger.Instance()
                                        .LogRecord("Row y=" + y + " non equal in/outs = " + turncount +
                                                   " - Model may have holes");
                                }
                            }
                        }
        ...
        public static List<Line2d> GetIntersecting2dYLines(int ypos, List<Line2d> all2dlines) 
        {
            ...
            return all2dlines.Where(ln => (ln.p1.y >= ypos && ln.p2.y <= ypos) || (ln.p2.y >= ypos && ln.p1.y <= ypos) || (ln.p2.y == ypos && ln.p1.y == ypos)).ToList();
        }
    }
}
