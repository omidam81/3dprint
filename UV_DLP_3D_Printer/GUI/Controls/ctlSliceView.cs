...
namespace UV_DLP_3D_Printer.GUI.Controls
{
    public partial class ctlSliceView : UserControl
    {
       // frmDLP m_frmdlp = null; // now that we support multiple monitors, we can't use this approach anymore
        ...
        {
            this.ctlToolTip1.SetToolTip(this.buttPreviewOnDisplay, ((DesignMode) ? "ViewSliceOnDeviceDisplay" : UVDLPApp.Instance().resman.GetString("ViewSliceOnDeviceDisplay", UVDLPApp.Instance().cul)));
            this.itemNumLayers.TitleText = ((DesignMode) ? "Layers" : UVDLPApp.Instance().resman.GetString("Layers", UVDLPApp.Instance().cul));
        }
