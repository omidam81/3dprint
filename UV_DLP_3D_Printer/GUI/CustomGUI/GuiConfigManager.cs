...
namespace UV_DLP_3D_Printer.GUI.CustomGUI
{
    public class GuiConfigManager
    {
       // public enum EntityType { Buttons, Panels, Decals } // not used
        ...
        Dictionary<String, ctlImageButtonEx> ButtonsEx;
       ...
       // IPlugin Plugin; // not used quite yet...
        ...
        public void AddButton(string name, ctlImageButtonEx ctl)
        {
            ButtonsEx[name] = ctl;
            //if ((ctl.Parent == null) && (mTopLevelControl != null))
            //    mTopLevelControl.Controls.Add(ctl);
        ...
        public void AddButton(ctlImageButtonEx ctl)
        {
            AddButton(ctl.Name, ctl);
        }
        ...
        public bool RemoveButtonEx(string name)
        {
            if ((name == null) || !Buttons.ContainsKey(name))
                return false;
            ButtonsEx.Remove(name);
            return true;
        }
        ...
        public void DumpDatabase(string fname)
        {
            ...
            sw.WriteLine(UVDLPApp.Instance().resman.GetString("Controls_", UVDLPApp.Instance().cul));
            foreach (KeyValuePair<String, Control> pair in Controls)
            {
                sw.WriteLine("  {0,-30}{1}", pair.Key, pair.Value.GetType().ToString());
            }
            sw.WriteLine("");
            sw.WriteLine(UVDLPApp.Instance().resman.GetString("Buttons_", UVDLPApp.Instance().cul));
...
//            butt.Visible = true;
            ...
            if (stl != null)
            {
                ...
                foreach (KeyValuePair<String, Control> pair in Controls)
                {
                    ...
                    else
                    {
                        ...
                        ApplyStyleRecurse(pair.Value, stl, stl.MaxDepth);
                    }
                }
            }
        ...
        public void ApplyStyleRecurse(Control ctl, GuiControlStyle ct, int maxDepth)
        {
            if (maxDepth == 0)
                return;
            ...
            foreach (Control subctl in ctl.Controls)
            {
                ...
                {
                    ApplyStyleRecurse(subctl, ct, maxDepth - 1);
                }
            }
        }
        ...
        void HandleControl(GuiConfigDB conf, GuiControl gctl)
        {
            ...
            if (gctl.action.IsExplicit())
            {
                ...
                else if (action.Contains("addto")) // this handles adding a new/existing control to a parent control
                {
                    ...
                    if (gctl.parent.IsExplicit() && (parentname != null) && (parentname.Length != 0))
                    {
                        ...
                        {
                            DebugLogger.Instance().LogWarning(UVDLPApp.Instance().resman.GetString("ControlParentNotFound", UVDLPApp.Instance().cul) + parentname);
                        }
                    }
                }
            }
            ...
            else
            {
                if (style != null)
                {
                    ApplyStyleRecurse(ct, style, style.MaxDepth);
                }
            }
        }
        ...
        string[]  VerifyArgs(Object varsobj, int count, string funcname)
        {
            ...
            if (vars.Length != count)
            {
                DebugLogger.Instance()
                    .LogError(
                        UVDLPApp.Instance().resman.GetString("WrongNumberOfArgumentsCalling", UVDLPApp.Instance().cul) +
                        funcname);
                return null;
            }
            return vars;
