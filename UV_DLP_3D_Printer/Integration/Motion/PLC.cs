...
namespace UV_DLP_3D_Printer.Integration.Motion
{
    public class PLCc
    {
        ...
        public void ConnectPLC(string Address, int Rack=0, int Slot=1)
        {
            int Result;
        ...
        public void SpreadCycle(int numLayers=0)
        {
            PLcDBRead();
        ...
        public void Heater(bool OnOff)
        {
            ...
            if (OnOff)
            {
                S7.SetBitAt(ref DB1, Heater_Start[0], Heater_Start[1], OnOff);
                PLcDBWrite();
            }
            else
            {
