...
namespace Engine3D
{
    public class Object3d
    {
        ...
        public Material material; // not used yet
        ...
        public Vector3 _transform = Vector3.Zero;
        ...
        public int NumPolys
        {
            get { return m_lstpolys.Count; }
        }

        public int NumPoints
        {
            get { return m_lstpoints.Count; }
        }
        ...
        public AxisIndicator m_axis { get; internal set; }
        ...
        public virtual void RenderGL(bool showalpha, bool selected, bool renderOutline, Color renderColor)
        {
            ...
            if (m_listid == -1)
            {
                ...
                if (this.m_wireframe > 0)
                {
                    GL.Begin(PrimitiveType.LineLoop);
                    foreach (Polygon poly in m_lstpolys)
                    {
                        poly.RenderGL(this.m_wireframe, showalpha,
                            renderColor != UVDLPApp.Instance().Engine3D.m_objectColor ? renderColor : poly.m_color, true);
                    }
                    GL.End();
                }
                else
                ...
                if (m_inSelectedList && renderOutline)
                {
                    ...
                    if (selected)
                        selcol = Color.FromArgb(255, 150, 150, 150);
                    foreach (Polygon poly in m_lstpolys)
                    {
            ...
            {
                var new_transform = (this.m_parent != null)
                    ? this.m_parent._transform + this._transform
                    : this._transform;
                GL.PushMatrix();
                GL.Translate(new_transform);
        ...
        public void FindMinMax()
        {
            ...
            m_min.x = this.m_lstpoints.Select(a => a.x).Min();
            m_min.y = this.m_lstpoints.Select(a => a.y).Min();
            m_min.z = this.m_lstpoints.Select(a => a.z).Min();
            m_max.x = this.m_lstpoints.Select(a => a.x).Max();
            m_max.y = this.m_lstpoints.Select(a => a.y).Max();
            m_max.z = this.m_lstpoints.Select(a => a.z).Max();
        ...
        public void ClearPolyTags()
        {
            ...
            {
                p.tag = Polygon.TAG_REGULAR;
                p.m_color = p.m_colorsource; // UVDLPApp.Instance().Engine3D.m_supportColor
            }
            InvalidateList();
        }
        ...
        */
        ...
        public virtual void Translate(float x, float y, float z, bool updateUndo = false, bool do_it_speedy = false)
        {
            if (this.m_inSelectedList && do_it_speedy)
            {
                lock (locker)
                {
                    _transform += new Vector3(x, y, z);
                    if (updateUndo)
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (line.ToLower().StartsWith("facet"))
                    {
                        ...
                        for (int idx = 0; idx < 3; idx++) //read the point, will break somehow on bad 4 vertext faces.
                        {
                            ...
                            poly.m_points[idx].x =
                                (float) float.Parse(toks[1].Trim(), System.Globalization.CultureInfo.InvariantCulture);
                            poly.m_points[idx].y =
                                (float) float.Parse(toks[2].Trim(), System.Globalization.CultureInfo.InvariantCulture);
                            poly.m_points[idx].z =
                                (float) float.Parse(toks[3].Trim(), System.Globalization.CultureInfo.InvariantCulture);
        ...
        internal void Move(float x, float y, float z, bool v1, bool v2)
        {
            this.Translate(x, y, z, v1, v2);
        }
                            ...
                            case "f": // a face              ﻿  ﻿  ﻿  ﻿  ﻿  ﻿  
                                if (parts.Length < 3)
                                {
                                    throw new FormatException(
                                        string.Format("Face found with less three indices (line {0})", curLineNo));
                                }
        ...
        private static float[] ParseVector(string[] parts)
        {
        ...
        private static int ParseFacePart(string part)
        {
            try
            {
        ...
        internal List<Polygon> IntersectForUpVector(Point3d origin, Vector3d direction)
        {
            //return this.m_lstpolys;
            //we can extend it later but for now we just return value for up direction
            if (direction.x == 0 && direction.y == 0 && direction.z != 0)
            {
                var returnedArray = new List<Polygon>();

                foreach (Polygon polygon in this.m_lstpolys)
                {
                    //if(pol)
                    ...
                    b1 =
                        (
                            origin.x < polygon.m_points[0].x ||
                            origin.x < polygon.m_points[1].x ||
                            origin.x < polygon.m_points[2].x
                            ) &&
                        (
                            origin.x > polygon.m_points[0].x ||
                            origin.x > polygon.m_points[1].x ||
                    ...
                    b2 =
                        (
                            origin.y < polygon.m_points[0].y ||
                            origin.y < polygon.m_points[1].y ||
                            origin.y < polygon.m_points[2].y
                            ) &&
                        (
                            origin.y > polygon.m_points[0].y ||
                            origin.y > polygon.m_points[1].y ||
        ...
        public void UpdateMeAsynchron()
        {
            Thread T = new Thread(new ThreadStart(UpdateMe));
            T.IsBackground = true;
        ...
        public void UpdateMe()
        {
            lock (locker)
            {
                ...
                {
                    foreach (Object3d sup in m_supports)
                        sup.Translate(_transform.X, _transform.Y, 0);
                }
                if (_transform.Z != 0)
                {
                    foreach (Object3d sup in m_supports)
                        if (sup is Support)
                            ((Support) sup).AddToHeight(_transform.Z);
                }
                m_center.Translate(_transform.X, _transform.Y, _transform.Z);
                m_min.Translate(_transform.X, _transform.Y, _transform.Z);
                m_max.Translate(_transform.X, _transform.Y, _transform.Z);

                foreach (Polygon p in m_lstpolys)
                {
                    p.UpdateMove(_transform.X, _transform.Y, _transform.Z);
                }
                m_listid = -1; // invalidate the list id
