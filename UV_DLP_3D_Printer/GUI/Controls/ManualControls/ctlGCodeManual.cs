...
namespace UV_DLP_3D_Printer.GUI.Controls.ManualControls
{
    public partial class ctlGCodeManual : ctlRFrame
    {
        ...
        public ctlGCodeManual()
        {
            ...
            UVDLPApp.Instance().m_callbackhandler.RegisterCallback("cmdLogGcode", LogGcode, typeof(string), ((DesignMode) ? "LogAllCommandsSentToMachine" :UVDLPApp.Instance().resman.GetString("LogAllCommandsSentToMachine", UVDLPApp.Instance().cul)));
            SetTexts();
        }

        private void SetTexts()
        {
            this.lblTitle.Text = ((DesignMode) ? "ManualGCodeCommands" : UVDLPApp.Instance().resman.GetString("ManualGCodeCommands", UVDLPApp.Instance().cul));
            this.cmdSendGCode.Text = ((DesignMode) ? "Send_" : UVDLPApp.Instance().resman.GetString("Send_", UVDLPApp.Instance().cul));
            this.label1.Text = ((DesignMode) ? "Sent" : UVDLPApp.Instance().resman.GetString("Sent", UVDLPApp.Instance().cul));
            this.label2.Text = ((DesignMode) ? "Received" : UVDLPApp.Instance().resman.GetString("Received", UVDLPApp.Instance().cul));
            this.cmdClear.Text = ((DesignMode) ? "Clear" : UVDLPApp.Instance().resman.GetString("Clear", UVDLPApp.Instance().cul));
            this.checkLogAll.Text = ((DesignMode) ? "LogAllGCodeSent" : UVDLPApp.Instance().resman.GetString("LogAllGCodeSent", UVDLPApp.Instance().cul));
        ...
        private void cmdSendGCode_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DebugLogger.Instance().LogRecord(((DesignMode) ? "CouldNotSendRawGCodeCommand" :UVDLPApp.Instance().resman.GetString("CouldNotSendRawGCodeCommand", UVDLPApp.Instance().cul)));
                }
            }
            catch (Exception ex)
            {
