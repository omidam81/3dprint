       ...
       // static int slcnt = 0; // slice / blank image counter
        ...
        private void ViewLayer(int layer, Bitmap image, int layertype)
        {
            try
            {
                ...
                {
                    DebugLogger.Instance().LogError(((DesignMode) ? "NullBMInDLPForm" :UVDLPApp.Instance().resman.GetString("NullBMInDLPForm", UVDLPApp.Instance().cul)));
                    return;
                }
        ...
        public Screen GetDLPScreen(bool reportError)
        {
            ...
            if ((dlpscreen == null) && reportError)
            {
                ...
                DebugLogger.Instance().LogRecord(((DesignMode) ? "CanTFindScreen" :UVDLPApp.Instance().resman.GetString("CanTFindScreen", UVDLPApp.Instance().cul)) + m_screenid);
            }
            return dlpscreen;
        }
