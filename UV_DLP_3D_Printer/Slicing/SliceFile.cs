        ...
        public Bitmap GetSliceImage(int layer, bool outline = false, bool bottomLayer = false) // 0 based index
        {
            ...
            try
            {
                if (m_mode == SFMode.eImmediate)
                {
                    ...
                    {
                        //float zlev = (float)(((double)layer + 0.5) * m_config.ZThick);
                        float zlev = (float)(((double) (layer - m_config.m_bottom_layers) + (m_config.ZThick / 2)) * m_config.ZThick);
                        //var t1 = DateTime.Now;
                        //for (int i = 0; i < 10000000; i++)
                        //{
                        //   UVDLPApp.Instance().m_slicer.SliceImmediate(zlev, outline);
                        //}

                        //var t2 = DateTime.Now - t1;

                        return UVDLPApp.Instance().m_slicer.SliceImmediate(zlev, outline, layer < m_config.m_bottom_layers);
                    }
                }
                else
                {
                    ...
                    {                        
                        ...
                        string fname = Path.GetFileNameWithoutExtension(path) + outlinename + String.Format("{0:0000}", layer) + ".bmp";
                        ZipEntry ze = m_zip[fname];
                        if (ze == null) { m_zip.Dispose(); return null;}
