...
namespace UV_DLP_3D_Printer.GUI.Controls
{
    public partial class ctl3DView : UserControl
    {
        ...
        public ctl3DView()
        {
            ...
            //guiconf.GetControl("numlayer",false).Height = 65;
            UpdateButtonList();
        ...
        {
            this.ctlToolTip1.SetToolTip(this.buttGLTop, ((DesignMode) ? "Home_" : UVDLPApp.Instance().resman.GetString("Home_", UVDLPApp.Instance().cul)));
            this.ctlToolTip1.SetToolTip(this.buttRedo, ((DesignMode) ? "Undo_" : UVDLPApp.Instance().resman.GetString("Undo_", UVDLPApp.Instance().cul)));
            this.ctlToolTip1.SetToolTip(this.buttUndo, ((DesignMode) ? "Undo_" : UVDLPApp.Instance().resman.GetString("Undo_", UVDLPApp.Instance().cul)));
            this.ctlToolTip1.SetToolTip(this.buttGlHome, ((DesignMode) ? "Home_" : UVDLPApp.Instance().resman.GetString("Home_", UVDLPApp.Instance().cul)));
           // ((DesignMode) ? "Home_" :UVDLPApp.Instance().resman.GetString("Home_", UVDLPApp.Instance().cul))
        }
            ...
            else
            {
                switch (ev)
                {
                    ...
                    case eAppEvent.eModelRemoved:
                        m_curslice = null; // invalidate the old slice
                        UpdateView();
                        break;
        ...
        {
            m_camera.ResetView(0, -600, 0, 20, 20);
            m_axisCam.ResetView(0, -300, 0, 20, 0);
            UpdateView();
        }
        ...
        private void SetupViewport()
        {
            ...
            try
            {
               ...
               // DebugLogger.Instance().LogInfo("Stencil depth: " + glControl1.GraphicsMode.Stencil.ToString());
                ...
                if (firstTime)
                {
                    gr2d.LoadTexture(Properties.Resource_s.cwtexture1_index);
                    gr2d.LoadFont("Arial18", Properties.Resource_s.Arial18_metrics);
                    LoadPluginGui();
                    //Visible = true;
                }
        ...
        private void DisplayFunc()
        {
            ...
            if (Render3dSpace)
            {
                ...
                //DrawISect();
                Render3dSlice();
                if (UVDLPApp.Instance().Engine3D.ShowIndicators)
                    RenderAxisIndicator();
        ...
        private void Render3dSlice()
        {
            try
            {
                 ...
                 * */
                if (m_curslice.m_segments == null) 
                {
                    DebugLogger.Instance().LogError("segments is null");
                }
                foreach (PolyLine3d ply in m_curslice.m_segments)
                {
                    try
                    {
                        if(ply != null) // why are the polylines in this list occassionally null?
                            ply.RenderGL();
                    }
                    catch (Exception ex) 
                    {
        ...
        private List<ISectData> TestHitTest(int X, int Y, bool checkselecteObjectOnly = false)
        {
            if (!loaded)
                return null;
            // String mess = "";
            ...
            List<Object3d> objList = checkselecteObjectOnly ? UVDLPApp.Instance().SelectedObjectList : UVDLPApp.Instance().Engine3D.m_objects;
        ...
        void ctl3DView_KeyUp(object sender, System.Windows.Forms.KeyEventArgs e)
        {
        ...
        private void glControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (Keyboard.GetState().IsKeyDown(Key.ShiftLeft) || Keyboard.GetState().IsKeyDown(Key.ShiftRight)) // if we're moving an object - shift key down
            {
                ...
                var t1 = DateTime.Now;
                List<ISectData> hits = TestHitTest(e.X, e.Y, true);
                ...
                foreach (ISectData dat in hits)
                {
                    ...
                    if (_firstTime)
                    {
                        oldx = dat.intersect.x;
                        oldy = dat.intersect.y;
                    ...
                    if (UVDLPApp.Instance().SelectedObject.tag == Object3d.OBJ_SUPPORT)
                    {
                        ...
                        if (sup.SelectionType == Support.eSelType.eWhole)
                        {
                            ...
                            if (dat.obj.tag == Object3d.OBJ_SEL_PLANE || dat.obj.tag == Object3d.OBJ_NORMAL)
                            {
                                //we should really try a top/ bottom intersection / scale to hieg
                                // move the support                                     
                                sup.Translate(
                                    (float)(dat.intersect.x - oldx),
                                    (float)(dat.intersect.y - -oldy),
                                    0.0f);
                    ...
                    {
                        //this.ParentForm.Text = dat.obj.tag.ToString();
                        if (dat.obj.tag == Object3d.OBJ_SEL_PLANE || dat.obj.tag == Object3d.OBJ_NORMAL)
                        {
                            //UVDLPApp.Instance().SelectedObject.Translate(
                            //    dat.intersect.x - oldhits[i].intersect.x,
                            //    dat.intersect.y - oldhits[i].intersect.y,
                            //    0
                            //    );
                            //dat.obj.SetColor(Color.Red);
                            if (_firstTime)
                            {
                                oldx = dat.intersect.x;
                                oldy = dat.intersect.y;
                            ...
                            {
                                var x = (float) (dat.intersect.x - oldx);
                                    //- UVDLPApp.Instance().SelectedObject.m_center.x);
                                var y = (float) (dat.intersect.y - oldy);
                                    // - UVDLPApp.Instance().SelectedObject.m_center.y);

                              

                                UVDLPApp.Instance().SelectedObject.UpdateMeAsynchron();
                                UVDLPApp.Instance().SelectedObject.Translate(x, y, .002f, true, true);

                                oldx = dat.intersect.x;
                                oldy = dat.intersect.y;

                                //UVDLPApp.Instance().SelectedObject.InvalidateList();
                            ...
                            //UVDLPApp.Instance().SelectedObject.RenderGL(true, true, false, Color.AntiqueWhite);
        ...
        private void buttGlHome_Click(object sender, EventArgs e)
        {
            if (m_modelAnimTmr != null)
                return;
            m_camera.ResetViewAnim(0, -600, 0, 20, 20);
            m_axisCam.ResetViewAnim(0, -300, 0, 20, 0);
        ...
        private void buttGLTop_Click(object sender, EventArgs e)
        {
            if (m_modelAnimTmr != null)
                return;
            m_camera.ResetViewAnim(0, -600, 0, 90, 20);
            m_axisCam.ResetViewAnim(0, -300, 0, 90, 0);
            m_modelAnimTmr = new Timer();
            m_modelAnimTmr.Interval = 25;
            m_modelAnimTmr.Tick += new EventHandler(m_modelAnimTmr_Tick);
            m_modelAnimTmr.Start();
        ...
        private void buttGlHome_Load(object sender, EventArgs e)
        {

        }

        void LoadSlice(int layer)//, bool view_for_3d_view = false)
        {
            Bitmap bmp = null;
...
//                double zlev = ((double) vscrollval*((m_sf != null)
  //                  ? m_sf.m_config.ZThick
    //                : UVDLPApp.Instance().m_buildparms.ZThick));
                double zlev = ((double)vscrollval * m_sf.m_config.ZThick);

                zlev = Math.Round(zlev, 3, MidpointRounding.ToEven);
                UVDLPApp.Instance().m_engine3d.SetClipHeight(zlev);
                ctlInfoItemZLev.DataText = zlev.ToString();
                ctlInfoItemZLev.Refresh();
                //ViewLayer(vscrollval, true);
