...
namespace UV_DLP_3D_Printer
{
    ...
    public class SliceBuildConfig
    {
        ...
        public Dictionary<string, FluidConfig> fluids;
        public Dictionary<string, PowderConfig> powders;
        ...
        public double m_feed_thickness;
        public double m_speed_spareading;
        public double m_speed_printing;
        public double m_bottom_layers;
        public double m_roller_speed;

        public double m_wall_thickness;
        ...
        public double m_fluidPrice;
        public double m_powderPrice;
        ...
        public void CreateDefault()
        {
            ...
            fluids = new Dictionary<string, FluidConfig>();
            selectedfluid = "default";
            fluids[selectedfluid] = new FluidConfig(selectedfluid);

            powders = new Dictionary<string, PowderConfig>();
            selectedpowder = "default";
            powders[selectedpowder] = new PowderConfig(selectedpowder);
        ...
        public bool UpdateCurrentPowder()
        {
            if (powders.ContainsKey(selectedpowder))
            {
                PowderConfig ic = powders[selectedpowder];
                ic.price = m_powderPrice;
                return true;
            }
            return false;
        }

        public bool UpdateCurrentFluid()
        {
            if (fluids.ContainsKey(selectedfluid))
            {
                FluidConfig ic = fluids[selectedfluid];
                ic.price = m_fluidPrice;
                return true;
            }
            return false;
        }
        ...
        public bool SetCurrentFluid(string fluid)
        {
            if (fluids.ContainsKey(fluid))
            {
                selectedfluid = fluid;
                FluidConfig ic = fluids[fluid];
                m_fluidPrice = ic.price;
                return true;
            }
            return false;
        }

        public bool SetCurrentPowder(string powder)
        {
            if (powders.ContainsKey(powder))
            {
                selectedpowder = powder;
                PowderConfig ic = powders[powder];
                m_powderPrice = ic.price;
                return true;
            }
            return false;
        }
        ...
        private void SaveInternal(ref XmlHelper xh)
        {
            ...
            xh.SetParameter(sbc, "SelectedPowder", selectedpowder);
            foreach (KeyValuePair<string, PowderConfig> entry in powders)
            {
                powders[entry.Key].Save(xh, sbc);
            }

            xh.SetParameter(sbc, "SelectedFluid", selectedfluid);
            foreach (KeyValuePair<string, FluidConfig> entry in fluids)
            {
                fluids[entry.Key].Save(xh, sbc);
            }
            ...
            xh.SetParameter(sbc, "FeedThickness", m_feed_thickness);
            xh.SetParameter(sbc, "WallThickness", m_wall_thickness);
            xh.SetParameter(sbc, "SpeedSpareading", m_speed_spareading);
            xh.SetParameter(sbc, "SpeedPrinting", m_speed_printing);
            xh.SetParameter(sbc, "Speed_printing", m_roller_speed);
            xh.SetParameter(sbc, "BottomLayers", m_bottom_layers);
        ...
        public override String ToString()
        {
            ...
            sb.Append(";(Slice thickness Z         = " + String.Format("{0:0.00000}", ZThick) + " mm )\r\n");
            sb.Append(";(Feed thickness Z         = " + String.Format("{0:0.00000}", m_feed_thickness) + " mm )\r\n");
            sb.Append(";(Speed spareading         = " + m_speed_spareading + " mm/s )\r\n");
            sb.Append(";(Speed printing         = " + m_speed_printing + " mm/s )\r\n");
            sb.Append(";(Bottom layers         = " + m_bottom_layers + " )\r\n");
            sb.Append(";(roller speed         = " + m_roller_speed + " rmp )\r\n");
            sb.Append(";(Wall thickness         = " + m_wall_thickness + " )\r\n");
        ...
        internal string AddNewPowder(string profileName)
        {
            if (powders.ContainsKey(profileName))
                return "Powder profile name already exists";
            PowderConfig ic = new PowderConfig(profileName);
            if ((selectedpowder != null) && inks.ContainsKey(selectedpowder))
                ic.CopyFrom(powders[selectedpowder]);
            powders[profileName] = ic;
            SetCurrentPowder(profileName);
            return "OK";
        }

        public string RemoveSelectedPowder()
        {
            if (powders.ContainsKey(selectedpowder))
            {
                powders.Remove(selectedpowder);
                if (inks.Count == 0)
                {
                    selectedpowder = null;
                    AddNewPowder("Default");
                    return "Powder profile reset to default|Attention";
                }
                else
                {
                    SetCurrentPowder(powders.First().Key);
                }
            }
            return "OK";
        }

        public string RemoveSelectedFluid()
        {
            if (fluids.ContainsKey(selectedfluid))
            {
                fluids.Remove(selectedInk);
                if (fluids.Count == 0)
                {
                    selectedfluid = null;
                    AddNewFluid("Default");
                    return "Fluid profile reset to default|Attention";
                }
                else
                {
                    SetCurrentFluid(fluids.First().Key);
                }
            }
            return "OK";
        }
