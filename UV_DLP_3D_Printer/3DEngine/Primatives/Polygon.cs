...
namespace Engine3D
{
    public class Polygon 
    {
        ...
        public Polygon() 
        {
            ...
            m_radius = 0.0f;
            m_colorsource = UVDLPApp.Instance().Engine3D.m_objectColor;
            m_color = m_colorsource;// UVDLPApp.Instance().Engine3D.m_supportColor
        ...
        public void RenderGL(int wireframe,bool alpha, Color clr, bool in_list = false) 
        {
            ...
            if (wireframe > 0)
            {
                ...
                if (!in_list) GL.Begin(PrimitiveType.LineLoop);//.LineStrip);
            }else
            {
                if (!in_list) GL.Begin(PrimitiveType.Triangles);
                if (tag == TAG_MARKDOWN)
                {
                    clr = Color.Red;
                }
