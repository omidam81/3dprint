...
namespace UV_DLP_3D_Printer.GUI.CustomGUI
{
    public partial class ctlObjectInfo : ctlAnchorable
    {
        ...
        public ctlObjectInfo()
        {
            ...
            SetTexts();
           // tName.ForeColor = Wcolor.ForeColor;
       
              //  BackColor = 
             //   layoutPanel.BackColor =
             //   ctlTitle1.BackColor =
             //   tName.BackColor = Wcolor.BackColor;
        }

        private void SetTexts()
        {
            this.ctlTitle1.Text = ((DesignMode) ? "ObjectInfo" : UVDLPApp.Instance().resman.GetString("ObjectInfo", UVDLPApp.Instance().cul));
            this.tName.Text = ((DesignMode) ? "ObjectInfo" : UVDLPApp.Instance().resman.GetString("ObjectInfo", UVDLPApp.Instance().cul));
            this.tVolume.TitleText = ((DesignMode) ? "Volume" : UVDLPApp.Instance().resman.GetString("Volume", UVDLPApp.Instance().cul));
            this.tCost.TitleText = ((DesignMode) ? "Cost" : UVDLPApp.Instance().resman.GetString("Cost", UVDLPApp.Instance().cul));
            this.tPoints.TitleText = ((DesignMode) ? "Points" : UVDLPApp.Instance().resman.GetString("Points", UVDLPApp.Instance().cul));
            this.tPolys.TitleText = ((DesignMode) ? "Polys" : UVDLPApp.Instance().resman.GetString("Polys", UVDLPApp.Instance().cul));
            this.tMin.TitleText = ((DesignMode) ? "Min" : UVDLPApp.Instance().resman.GetString("Min", UVDLPApp.Instance().cul));
            this.tMax.TitleText = ((DesignMode) ? "Max" : UVDLPApp.Instance().resman.GetString("Max", UVDLPApp.Instance().cul));
            this.tSize.TitleText = ((DesignMode) ? "Size" : UVDLPApp.Instance().resman.GetString("Size", UVDLPApp.Instance().cul));
        }
        ...
        private void AdjustHeight(Control ctl, float newHeight)
        {
            ...
            ctl.Font = new Font(((DesignMode) ? "Arial" :UVDLPApp.Instance().resman.GetString("Arial", UVDLPApp.Instance().cul)), newHeight / 1.5f, GraphicsUnit.Pixel);
        }
        ...
        public void FillObjectInfo(Object3d selobj)
        {
            ...
            tName.Text = buttScene.Checked ? ((DesignMode) ? "Scene_" :UVDLPApp.Instance().resman.GetString("Scene_", UVDLPApp.Instance().cul)) : selobj.Name;
