...
namespace UV_DLP_3D_Printer.GUI
{
    public partial class frmExport : Form
    {
        ...
        private void ShowMessage(string msg)
        {
            ...
            string title = ((DesignMode) ? "Info" :UVDLPApp.Instance().resman.GetString("Info", UVDLPApp.Instance().cul));
            string text = "";
            if (message.Length >= 2)
            {
                title = message[0];
        ...
        private void buttExport_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(((DesignMode) ? "SelectExportTypeFirst" :UVDLPApp.Instance().resman.GetString("SelectExportTypeFirst", UVDLPApp.Instance().cul)), ((DesignMode) ? "Attention" :UVDLPApp.Instance().resman.GetString("Attention", UVDLPApp.Instance().cul)));
                return;
            }
        ...
        {
            for (int i = 0; i < comboExportType.Items.Count; i++)
            {
                if (comboExportType.Items[i].ToString().StartsWith(type))
                {
                    comboExportType.SelectedIndex = i;
                    break;
                }
            }
        }
        ...
        {
            this.label1.Text = ((DesignMode) ? "ExportType" :UVDLPApp.Instance().resman.GetString("ExportType", UVDLPApp.Instance().cul));
            this.buttCancel.Text = ((DesignMode) ? "Cancel" :UVDLPApp.Instance().resman.GetString("Cancel", UVDLPApp.Instance().cul));
            this.buttExport.Text = ((DesignMode) ? "Export_" :UVDLPApp.Instance().resman.GetString("Export_", UVDLPApp.Instance().cul));
        }
    }
}
